...
export function calculateShapeTransforms(
  ...
  const leftFootRotationDegrees = directionToDegrees(
    currentStance.leftFootRotation
  );
  const rightFootRotationDegrees = directionToDegrees(
    currentStance.rightFootRotation
  );

  const toSvgX = (mathX: number) =>
    (mathX + offsetX) * canvasDims.feetDistance + centerX;
  const toSvgY = (mathY: number) =>
    centerY - (mathY + offsetY) * canvasDims.feetDistance;
  ...
  const leftFootTotalRotation =
    (rotationDegrees + leftFootRotationDegrees) % 360;
  const rightFootTotalRotation =
    (rotationDegrees + rightFootRotationDegrees) % 360;

  return {
    leftFootGroup: {
      x: leftFootSvgX,
      y: leftFootSvgY,
      rotate: leftFootTotalRotation
    },
    rightFootGroup: {
      x: rightFootSvgX,
      y: rightFootSvgY,
      rotate: rightFootTotalRotation
    },
    cogPointer: { x: cogSvgX, y: cogSvgY, rotate: rotationDegrees }
  };
}

export function applyPivotLogic(
  nextConfig: PersonConfig,
  lastConfig: PersonConfig,
  pivot: 'right' | 'left' | undefined,
  fromCoords: ShapeTransforms,
  canvasDims: CanvasDims
) {
  ...
  if (pivot === 'left' || pivot === 'right') {
    const fromPivotCoords =
      pivot === 'left'
        ? { cx: fromCoords.leftFootGroup.x, cy: fromCoords.leftFootGroup.y }
        : { cx: fromCoords.rightFootGroup.x, cy: fromCoords.rightFootGroup.y };
    ...
    const toOffsetX =
      (fromPivotCoords.cx - centerX) / canvasDims.feetDistance - toPivotMath.x;
    const toOffsetY =
      (centerY - fromPivotCoords.cy) / canvasDims.feetDistance - toPivotMath.y;
...
export function generatePersonShapes(
  personConfig: PersonConfig,
  canvasDims: CanvasDims,
  rightFootSvgContent: string,
  headSvgContent: string
): string {
  ...
  shapesSvg += `<g id="leftFootGroup" class="fill-green-trees-700" transform="translate(${transforms.leftFootGroup.x}, ${transforms.leftFootGroup.y}) rotate(${transforms.leftFootGroup.rotate})">
        <g transform="scale(${scaleFactor}, ${scaleFactor}) scale(-1, 1) translate(${svgOffsetX}, ${svgOffsetY})">
            ${rightFootSvgContent}
        </g>
    </g>`;

  shapesSvg += `<g id="rightFootGroup" class="fill-green-trees-700" transform="translate(${transforms.rightFootGroup.x}, ${transforms.rightFootGroup.y}) rotate(${transforms.rightFootGroup.rotate})">
  ...
  shapesSvg += `<g id="cog" class="fill-orange-leaves-700" transform="translate(${transforms.cogPointer.x}, ${transforms.cogPointer.y}) rotate(${transforms.cogPointer.rotate})">
...
export function createBodyPartMovementAnim(
  targetId: string,
  fromPos: BodyPartPosition,
  toPos: BodyPartPosition,
  duration: number
...
function createBodyPartAnimations(
  fromCoords: ShapeTransforms,
  ...
  stepAnims.push(
    createBodyPartMovementAnim(
      '#leftFootGroup',
      fromCoords.leftFootGroup,
      toCoords.leftFootGroup,
  ...
  stepAnims.push(
    createBodyPartMovementAnim(
      '#rightFootGroup',
      fromCoords.rightFootGroup,
      toCoords.rightFootGroup,
      stepAnimationDuration
    )
  );
  stepAnims.push(
...
export function processBodyAnimationsToAnimationData(
  ...
  const bodyPartAnims = createBodyPartAnimations(
    fromCoords,
    toCoords,
    stepAnimationDuration
  );
  ...
  if (bodyPartAnims.length > 0) {
    ...
    result.push({
      ...
      targets: bodyPartAnims.map((anim) => ({
        target: anim.targets,
        cfg: anim.options
      }))
    });
