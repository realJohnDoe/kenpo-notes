...
function calculateShapeTransforms(
  personConfig: any,
  canvasWidth: number,
  canvasHeight: number,
  unit: number
...
export function generatePersonShapes(
  ...
  const transforms = calculateShapeTransforms(
    personConfig,
    canvasWidth,
    canvasHeight,
    unit
...
export function createCircleAnim(
  targetId: string,
...
export function createPointerAnim(
  targetId: string,
  fromPos: { x: number; y: number; rotate: number },
  toPos: { x: number; y: number; rotate: number },
  duration: number
) {
...
export function generateAnimationTimeline(
  cfg: any,
  canvasWidth: number,
  canvasHeight: number,
  unitSize: number
): { timelineData: any[]; labelsData: any[] } {
  ...
  if (cfg.steps.length > 1) {
    ...
    for (let i = 0; i < cfg.steps.length - 1; i++) {
      const fromCoords = calculateShapeTransforms(
        lastConfig,
        canvasWidth,
        canvasHeight,
        unitSize
      ...
      const stepAnimationDuration =
        baseAnimationDuration *
        (toStep.duration !== undefined ? toStep.duration : 1);
      ...
      if (pivot === 'left' || pivot === 'right') {
        const fromPivotCoords =
          pivot === 'left'
            ...
            : {
                cx: fromCoords.rightFootGroup.x,
                cy: fromCoords.rightFootGroup.y
              };
        const toStance = stances[toStep.stance];

        let toPivotMath =
          pivot === 'left' ? toStance.leftFoot : toStance.rightFoot;
      ...
      const toCoords = calculateShapeTransforms(
        nextConfig,
        canvasWidth,
        canvasHeight,
        unitSize
      ...
      if (
        toStep.labels &&
        Array.isArray(toStep.labels) &&
        toStep.labels.length > 0
      ) {
...
export function computeAnimationData(timelineData: any[]): AnimationData[] {
  ...
  for (const step of timelineData) {
    const bodyPartAnims = step.anims.filter(
      (anim: any) =>
        anim.targets.startsWith('#') && !anim.targets.includes('label')
    );
    const labelAnims = step.anims.filter((anim: any) =>
      anim.targets.includes('label')
    );
    ...
    if (bodyPartAnims.length > 0) {
      ...
      animationDataList.push({
        ...
        targets: bodyPartAnims.map((anim: any) => ({
          target: anim.targets,
          cfg: anim.options
        }))
      });
    ...
    if (labelAnims.length > 0) {
      ...
      for (const labelAnim of labelAnims) {
        const labelTotalDuration = labelAnim.options.opacity.reduce(
